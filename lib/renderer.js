...
function checkConfig(config, md) {
    ...
    checkValue(config, _res, 'highlight', function(str, lang) {
        ...
        if (lang && hljs.getLanguage(lang)) {
            try {
                result = hljs.highlight(lang, str, true).value.replace(
                    /\r\n|\r/g,
                    '\n'
                ).replace(/class="hljs-/g, 'class="');
            } catch (__) {}
        }
        else {

            result = utils.escapeHtml(str);
        }

        var output = '<figure class="highlight ' + lang + '"><table><tr><td class="gutter"><pre>';
        var lines = result.length - result.replace(/\n/g, '').length;
        var tmp = [];
        for (var i = 1; i <= lines; i++) {
            tmp.push('<span class="line">' + i + '</span>');
        }
        output += tmp.join('<br>');
        ...
        output += '</span></pre></td></tr></table></figure>';
        return output;
    }, function(str, lang) {
...
module.exports = function(data, options) {
    ...
    return md.render(data.text).replace(
        /<pre><code( class="\w+")?><figure/g,
        '<figure'
    ).replace(
        /<\/figure><\/code><\/pre>/g,
        '</figure>'
